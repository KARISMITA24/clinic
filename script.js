    ...
    get(ref(db,'patients/')).then((Id)=>{
      ...
      for(var a=ids.length-1;a>=0;a--){
        get(ref(db,'patients/'+ids[a])).then((patientList)=>{
          var patientIDs=patientList.val();
          var patient=Object.values(patientIDs);
          var li=document.createElement('li');
          li.setAttribute('class','li1');
        li.innerHTML=`<div class=listDivs>
        <table>
            <tr><td>Date</td><td>:</td><td>${patient[2]}</td></tr>
            <tr><td>Name</td><td>:</td><td>${patient[4]}</td></tr>
            <tr><td>Age</td><td>:</td><td>${patient[0]}</td></tr>
            <tr><td>Gender</td><td>:</td><td>${patient[3]}</td></tr>
        </table>
            <div class='check'>${patient[1]}</div>
        </div><br>`;
        list.appendChild(li);
        })
      }
    ...
    get(ref(db,'users/')).then((snapshot)=>{
      ...
      for(var i=ids.length-1;i>=0;i--){
        get(ref(db,'users/'+ids[i])).then((snapChild)=>{
          ...
          var nam=Object.values(dataChild)[3];
          var email=Object.values(dataChild)[2];
          var password=Object.values(dataChild)[4];
          var designation=Object.values(dataChild)[1];
